...
export function parseReceiptText(text) {
  ...
  let merchant = ''
  const skipWords =
    /\b(total|amount|receipt|invoice|tax|pin|date|time|tel|phone|customer|cashier|change|balance|ksh|kes|usd|\$|subtotal|qty|unit|price|payment|bill to|ship to|description|gst|vat)\b/i
  ...
  // Currency detection
  const currencyMatch = fullText.match(/\b(KSH|KES|USD|\$)\b/i)
  const currency = currencyMatch ? currencyMatch[1].toUpperCase() : 'KSH'
